...
export async function handleBranchesToCommits(
  ...
  const message = Object.entries(branchesToCommits).map(([branch, status]) => {
    ...
    return [
      ...
      `   🏠 ${status.commitsExclusiveToLocalBranch} commit(s) locally that the remote branch doesn't have`,
...
export async function getReleaseBranches() {
  const releaseBranches = unwrap(await $`git branch --list release/*`)
    .split('\n')
    .map((branch) => branch.trim())

  releaseBranches.sort((releaseBranchA, releaseBranchB) => {
